package backupclientv2

import (
	"bytes"
	"encoding/json"
	"fmt"
	"io"
	"net/http"
	"net/url"
)

// This code was automatically generated by github.com/mittwald/api-client-go-builder.
// DO NOT EDIT.

// UpdateProjectBackupDescriptionRequest models a request for the
// 'backup-update-project-backup-description' operation. See [1] for more
// information.
//
// Change the description of a ProjectBackup.
//
// [1]:
// https://developer.mittwald.de/docs/v2/reference/backup/backup-update-project-backup-description
type UpdateProjectBackupDescriptionRequest struct {
	Body            UpdateProjectBackupDescriptionRequestBody
	ProjectBackupID string
}

// BuildRequest builds an *http.Request instance from this request that may be used
// with any regular *http.Client instance.
func (r *UpdateProjectBackupDescriptionRequest) BuildRequest() (*http.Request, error) {
	body, contentType, err := r.body()
	if err != nil {
		return nil, err
	}

	req, err := http.NewRequest(http.MethodPatch, r.url(), body)
	if err != nil {
		return nil, err
	}
	req.Header.Set("Content-Type", contentType)
	return req, nil
}

func (r *UpdateProjectBackupDescriptionRequest) body() (io.Reader, string, error) {
	out, err := json.Marshal(&r.Body)
	if err != nil {
		return nil, "", fmt.Errorf("error while marshalling JSON: %w", err)
	}
	return bytes.NewReader(out), "application/json", nil
}

func (r *UpdateProjectBackupDescriptionRequest) url() string {
	return fmt.Sprintf("/v2/project-backups/%s/description", url.PathEscape(r.ProjectBackupID))
}

func (r *UpdateProjectBackupDescriptionRequest) query() url.Values {
	return nil
}
