package databaseclientv2

import (
	"fmt"
	"io"
	"net/http"
	"net/url"
)

// This code was automatically generated by github.com/mittwald/api-client-go-builder.
// DO NOT EDIT.

// GetMysqlDatabaseRequest models a request for the 'database-get-mysql-database'
// operation. See [1] for more information.
//
// Get a MySQLDatabase.
//
// Returns a MySQLDatabase resource and its current status.
//
// Note that establishing a connection requires the linked user to have a status of
// `ready`; relying solely on the database status is insufficient.
//
// [1]:
// https://developer.mittwald.de/docs/v2/reference/database/database-get-mysql-database
type GetMysqlDatabaseRequest struct {
	MysqlDatabaseID string
}

// BuildRequest builds an *http.Request instance from this request that may be used
// with any regular *http.Client instance.
func (r *GetMysqlDatabaseRequest) BuildRequest(reqEditors ...func(req *http.Request) error) (*http.Request, error) {
	body, contentType, err := r.body()
	if err != nil {
		return nil, err
	}

	req, err := http.NewRequest(http.MethodGet, r.url(), body)
	if err != nil {
		return nil, err
	}
	req.Header.Set("Content-Type", contentType)
	for _, editor := range reqEditors {
		if err := editor(req); err != nil {
			return nil, err
		}
	}
	return req, nil
}

func (r *GetMysqlDatabaseRequest) body() (io.Reader, string, error) {
	return nil, "", nil
}

func (r *GetMysqlDatabaseRequest) url() string {
	u := url.URL{
		Path: fmt.Sprintf("/v2/mysql-databases/%s", url.PathEscape(r.MysqlDatabaseID)),
	}
	return u.String()
}

func (r *GetMysqlDatabaseRequest) query() url.Values {
	return nil
}
