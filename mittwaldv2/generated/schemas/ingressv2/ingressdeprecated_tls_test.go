package ingressv2_test

// This code was automatically generated by github.com/mittwald/api-client-go-builder.
// DO NOT EDIT.

import (
	"encoding/json"

	"github.com/mittwald/api-client-go/mittwaldv2/generated/schemas/ingressv2"
	. "github.com/onsi/ginkgo/v2"
	. "github.com/onsi/gomega"
)

var _ = Describe("IngressDeprecatedTLS", func() {
	When("unmarshaling from JSON", func() {
		It("should unmarshal into AlternativeTlsAcmeDeprecated", func() {
			exampleJSON := []byte("{\"acme\":true}")

			sut := ingressv2.IngressDeprecatedTLS{}
			Expect(json.Unmarshal(exampleJSON, &sut)).To(Succeed())
			Expect(sut.Validate()).To(Succeed())
			Expect(sut.AlternativeTlsAcmeDeprecated).NotTo(BeNil())
		})
		It("should unmarshal into AlternativeTlsCertificate", func() {
			exampleJSON := []byte("{\"certificateId\":\"7a9d8971-09b0-4c39-8c64-546b6e1875ce\"}")

			sut := ingressv2.IngressDeprecatedTLS{}
			Expect(json.Unmarshal(exampleJSON, &sut)).To(Succeed())
			Expect(sut.Validate()).To(Succeed())
			Expect(sut.AlternativeTlsCertificate).NotTo(BeNil())
		})
	})
})
