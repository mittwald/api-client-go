package activitylogv2

import "fmt"

// This code was automatically generated by github.com/mittwald/api-client-go-builder.
// DO NOT EDIT.

// This data type was generated from the following JSON schema:
// type: "object"
// properties:
//    "changes":
//        type: "object"
//        additionalProperties: false
//    "name":
//        type: "string"
//        enum:
//            - "app.failed"
//    "parameters":
//        type: "object"
//        properties:
//            "appInstallation": {"$ref": "#/components/schemas/de.mittwald.v1.activitylog.LinkedParameterProperty"}
//            "error": {"$ref": "#/components/schemas/de.mittwald.v1.activitylog.ParameterProperty"}
//        required:
//            - "appInstallation"
// required:
//    - "name"
//    - "changes"
//    - "parameters"

type AppInstallationFailed struct {
	Changes    AppInstallationFailedChanges    `json:"changes"`
	Name       AppInstallationFailedName       `json:"name"`
	Parameters AppInstallationFailedParameters `json:"parameters"`
}

func (o *AppInstallationFailed) Validate() error {
	if err := o.Changes.Validate(); err != nil {
		return fmt.Errorf("invalid property changes: %w", err)
	}
	if err := o.Name.Validate(); err != nil {
		return fmt.Errorf("invalid property name: %w", err)
	}
	if err := o.Parameters.Validate(); err != nil {
		return fmt.Errorf("invalid property parameters: %w", err)
	}
	return nil
}
